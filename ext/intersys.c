...
VALUE cTime, cDate;
VALUE cIntersysException, cObjectNotFound, cMarshallError, cUnMarshallError, cConnectionError;
VALUE cStatus;
...
void Init_intersys_bindings() {
	...
	mIntersys = rb_const_get(rb_cObject, rb_intern("Intersys"));
	...
	cIntersysException = rb_const_get(mIntersys, rb_intern("IntersysException"));
	...
	cConnectionError = rb_const_get(mIntersys, rb_intern("ConnectionError"));
	...
	rb_define_method(cDatabase, "reconnect!", intersys_base_reconnect, 0);
	...
	rb_define_method(cQuery, "column_count", intersys_query_column_count, 0);
